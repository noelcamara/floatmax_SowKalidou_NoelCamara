...
public class MaxFlow implements Algorithm {
    ...
    public void init(Graph graph) {
        ...
        for (int i = 0; i < graph.getEdgeCount(); i++) {
            ...
            Edge e1 = network.addEdge(e.getId(), e.getSourceNode().getId(),
                    e.getTargetNode().getId(), true);
            e1.setAttribute("cap", e.hasNumber(capacityAttribute) ?
                    e.getNumber(capacityAttribute) : Double.POSITIVE_INFINITY);
            e1.setAttribute("flow", 0);

            Edge e2 = network.addEdge(e.getId() + "_rev", e.getTargetNode().getId(),
    ...
    private void bfs() {
        ...
        while (!queue.isEmpty()) {
            ...
            n.leavingEdges().forEach(e -> {
                ...
                if (neighbor != source && !neighbor.hasAttribute("arc")
                        && cap > flow) {
    ...
    private void augmentFlow() {
        ...
        for (Edge e = (Edge) sink.getAttribute("arc"); e != null;
             e = (Edge) e.getSourceNode().getAttribute("arc")) {
